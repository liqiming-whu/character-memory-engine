      ...
      (function(todo, tIdx) {
        ...
        pendingItems.push(UI.Surface({ fillMaxWidth: true, shape: { cornerRadius: 8 }, containerColor: colors.surfaceVariant, padding: 8 }, [
          UI.Row({ fillMaxWidth: true, verticalAlignment: 'center' }, [
            UI.Surface({ width: 22, height: 22, shape: { cornerRadius: 11 }, containerColor: colors.surface, border: { width: 2, color: colors.error }, onClick: function() { return toggleTodoItem(todo.id !== undefined ? todo.id : realIdx); } }, []),
            UI.Spacer({ width: 8 }),
            UI.Column({ weight: 1 }, [
            ...
            makeDeleteBtn('todos', (todo.id !== undefined ? todo.id : realIdx), key),
          ]),
        ]));
    ...
    for (var tdi = 0; tdi < completedTodos.length; tdi++) {
      (function(todo, tIdx) {
        ...
        doneItems.push(UI.Surface({ fillMaxWidth: true, shape: { cornerRadius: 8 }, containerColor: colors.primaryContainer, alpha: 0.5, padding: 8 }, [
          UI.Row({ fillMaxWidth: true, verticalAlignment: 'center' }, [
            UI.Surface({ width: 22, height: 22, shape: { cornerRadius: 11 }, containerColor: colors.primary, onClick: function() { return toggleTodoItem(todo.id !== undefined ? todo.id : realIdx); } }, [
