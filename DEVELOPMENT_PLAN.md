...
| 官方 Memory 协同与工作区治理专项计划 | 独立子计划：官方 Memory 未命中后的 CME 兜底检索、双源统一检索、自动提取与准入写入、CME 与工作区边界、分阶段实施和回滚；状态为规划中，未实施 | `docs/CME_官方Memory协同与工作区治理计划.md` |
...
### 首次进入 UI 卡顿——部署/worker 初始化阶段加载提示（待评估，2026-08-14 入档）

- 现象：首次安装或重启 Operit 后进入 CME，UI 先卡住一段时间（数秒）再恢复正常；推测为后台正在**安装依赖**（首次部署 venv + pip 装 onnxruntime/tokenizers 等）+ **初始化**（worker 拉起、模型加载）。
- 影响：首次进入无任何进度反馈，用户会误以为卡死。
- 优化方向（待评估，先复现再动手）：
  1. 初始化阶段 UI 增加**加载条/静态占位 + 分阶段提示**：「正在安装依赖」（deploy_install 进行中）→「正在拉起 worker」（safeAutoLaunch）→「正在加载模型」（onnx 加载）；
  2. 复用 `deploy_status` 已上报字段（venv_ok / worker_running / model_ok / vec_available）驱动阶段提示，避免额外探测；
  3. UI 初始化在 worker 未就绪时快速返回「初始化中」状态，不阻塞、不无限转圈（与「空壳重试优化」「转圈优化」条目协调，避免重复改造）。
- 约束：不动 P0-C1（onAppCreate health-only）与 safeAutoLaunch 单飞语义；不引入新竞态。
- 状态：待评估（未排期）。
